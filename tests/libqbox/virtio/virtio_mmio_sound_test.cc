...
/*
 * VirtIO MMIO Sound Device Basic Test with Device Reset Iterations
 *
 * This test performs multiple iterations of VirtIO sound device initialization and basic setup with device resets:
 * 1. Creates a VirtIO sound device with 'none' audio backend (no audio hardware dependencies)
 ...
 */
class VirtioMmioSoundTest : public TestBench
{
public:
private:
    QemuInstanceManager m_inst_manager;
    QemuInstance m_qemu_inst;
    std::unique_ptr<virtio_mmio_sound> m_sound_device;
    gs::router<> m_router;
...
public:
    SC_HAS_PROCESS(VirtioMmioSoundTest);

    VirtioMmioSoundTest(const sc_core::sc_module_name& n)
        : TestBench(n)
    ...
    {
        // Configure CCI parameters for the VirtIO sound device
        std::string full_name = std::string(name()) + ".virtio_sound";
        ...
        SCP_INFO(SCMOD) << "Creating VirtIO sound device test with 'none' audio backend (no actual audio)";

        // Create the VirtIO sound device with 'none' driver to avoid audio hardware dependencies
        m_sound_device = std::make_unique<virtio_mmio_sound>("virtio_sound", m_qemu_inst, "none");

        // Connect device to router
        m_router.add_target(m_sound_device->socket, VIRTIO_BASE, VIRTIO_SIZE);
        m_initiator.socket.bind(m_router.target_socket);

        // Start iterative sound device test with resets
        SC_THREAD(run_iterative_tests);
    }

    void run_iterative_tests()
    {
        SCP_INFO(SCMOD) << "Starting iterative VirtIO sound device tests with device resets (" << NUM_ITERATIONS
                        << " iterations)";
    ...
    {
        SCP_INFO(SCMOD) << "Starting VirtIO sound device initialization test for iteration " << m_current_iteration;
        ...
        try {
            ...
            // Test 3: Verify device ID (sound device = 25)
            SCP_INFO(SCMOD) << "Test 3: Verifying device ID for sound device";
            uint32_t device_id;
            ...
            if (device_id != VIRTIO_ID_SOUND) {
                SCP_ERR(SCMOD) << "Test 3 FAILED: Expected sound device ID " << VIRTIO_ID_SOUND << ", got "
                               << device_id;
                return false;
            }

            SCP_INFO(SCMOD) << "Test 3 PASSED: Sound device ID verified (" << device_id << ")";
            ...
            SCP_INFO(SCMOD) << "All VirtIO sound device tests passed for iteration " << m_current_iteration;
            return true;

        } catch (const std::exception& e) {
            SCP_ERR(SCMOD) << "Exception during VirtIO sound device testing in iteration " << m_current_iteration
                           << ": " << e.what();
            return false;
        }
    }
    ...
    virtual void end_of_simulation() override
    {
        ...
        SCP_INFO(SCMOD) << "Iterative VirtIO sound MMIO test with device resets completed successfully";
        SCP_INFO(SCMOD) << "Successfully completed " << m_successful_iterations << "/" << NUM_ITERATIONS
                        << " iterations";
    }
};

int sc_main(int argc, char* argv[]) { return run_testbench<VirtioMmioSoundTest>(argc, argv); }
