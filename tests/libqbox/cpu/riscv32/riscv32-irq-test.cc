...
class CpuRiscv32TimerIrqTest : public CpuRiscvTestBench<cpu_riscv32, CpuTesterMmio>
{
     ...
     */
    void load_firmware_binary(uint32_t mmio_addr, uint32_t aclint_addr,
                             uint32_t timer_period, uint32_t setup_val, uint32_t handled_val, uint32_t complete_val)
    {
        SCP_DEBUG(SCMOD) << "load_firmware_binary called with mmio_addr=0x" << std::hex << mmio_addr
                         << ", aclint_addr=0x" << aclint_addr
                         << ", timer_period=" << std::dec << timer_period
        ...
        // Patch the constants in the binary (last 24 bytes: mmio_addr, aclint_addr, timer_period, setup_val, handled_val, complete_val)
        TEST_ASSERT(size >= 24);
...
public:
        ...
        , time_elapsed_ms(0)
    {
        ...
        SCP_INFO(SCMOD) << "Constructor: After setting timer params, aclint_mtimer.p_num_harts=" << (unsigned int)aclint_mtimer.p_num_harts;
        ...
        SCP_INFO(SCMOD) << "  ACLINT MTimer at 0x" << std::hex << ACLINT_MTIMER_BASE << " (size 0x" << ACLINT_MTIMER_SIZE << ")";
        ...
        load_firmware_binary(
            static_cast<uint32_t>(CpuTesterMmio::MMIO_ADDR),     // MMIO communication address
    ...
    void before_end_of_elaboration() override
    {
        ...
        SCP_INFO(SCMOD) << "After parent elaboration - Timer IRQ binding: m_cpus.size()=" << m_cpus.size()
                         << ", p_num_cpu=" << (int)p_num_cpu
                         << ", timer_irq.size()=" << aclint_mtimer.timer_irq.size();
        ...
        aclint_mtimer.timer_irq.init(p_num_cpu, [](const char* n, size_t i) { return new QemuInitiatorSignalSocket(n); });
        SCP_INFO(SCMOD) << "Forced timer_irq init, size=" << aclint_mtimer.timer_irq.size();

        // Connect timer IRQ outputs to CPU timer interrupt inputs
        for (int i = 0; i < m_cpus.size() && i < aclint_mtimer.timer_irq.size(); i++) {
            auto& cpu = m_cpus[i];
            aclint_mtimer.timer_irq[i].bind(cpu.irq_timer_in);
            SCP_INFO(SCMOD) << "Connected ACLINT MTimer timer_irq[" << i << "] to cpu[" << i << "].irq_timer_in";
        }
    }
    ...
    virtual void mmio_write(int id, uint64_t addr, uint64_t data, size_t len) override
    {
        SCP_INFO(SCMOD) << "MMIO write: addr=0x" << std::hex << addr << " data=0x" << data
                        << " (expect 1=timer_setup, 2=timer_irq_handled, 3=test_complete), timer_irq_count="
                        << std::dec << timer_interrupt_count;
...
int sc_main(int argc, char* argv[]) {
    return run_testbench<CpuRiscv32TimerIrqTest>(argc, argv);
}
