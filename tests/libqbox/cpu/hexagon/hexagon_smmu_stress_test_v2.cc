...
protected:
    void set_hex_firmware(const char* assembly, uint64_t addr = 0)
    {
        ks_engine* ks;
        ks_err err;
        size_t size, count;
        ...
        err = ks_open(KS_ARCH_HEXAGON, KS_MODE_LITTLE_ENDIAN, &ks);
        ...
        if (ks_asm(ks, assembly, addr, &fw, &size, &count) != KS_ERR_OK || size == 0) {
            ...
            SCP_INFO() << assembly;
            TEST_FAIL("Unable to assemble the Hexagon firmware\n");
        }
...
public:
    ...
    void generate_tester_controlled_firmware()
    {
        ...
        set_hex_firmware(boot_buf, BOOT_ADDR);
        ...
        static constexpr const char* DIAGNOSTIC_HANDLER = R"(
            ...
            diagnostic_error:
                .word 0x6230c000 // htid to get CPU ID
                r17 = ##0x%08)" PRIx64 R"(
                r1 = #0x100
                r1 = mpyi(r1, r16)
                r17 = add(r17, r1)
                ...
                memw(r17 + #0x28) = r2
                r0 = #1
                .word 0x6440c000  // wait (halt)
            diagnostic_loop:
                .word 0x6440c000  // wait
        ...
        set_hex_firmware(main_buf, MAIN_FIRMWARE_ADDR);
